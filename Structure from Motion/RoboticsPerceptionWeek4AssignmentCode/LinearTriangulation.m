...
X = zeros(N,3);
for i = 1:N
    x1_cur = Vec2Skew([x1(i,:)'; 1]);
    x2_cur = Vec2Skew([x2(i,:)'; 1]);
    A = [x1_cur*P1; x2_cur*P2];
    % solve for the least square problem, type 2
    [~,~,V] = svd(A);
    X_cur = V(:,end) / V(end,end);  % homogenous coordinate system (last be 1)
    X_cur = X_cur';
    X(i,:) =  X_cur(1:3);
end
